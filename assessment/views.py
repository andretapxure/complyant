...
from assessmentitems.models import AssessmentItems, ComplianceLevel
...
def assessment_fill(request, id):
    assessmentObj = Assessment.objects.get(pk=id)
    assessmentitems = AssessmentItems.objects.filter(
        assessment=assessmentObj)
    assessmentitems_forms = []
    for assessmentitem in assessmentitems:
        assessmentitems_forms.append(assessmentitem)
    context = {'assessmentitems_forms': assessmentitems_forms}
    return render(request, "assessment/assessment_fill.html", context)
